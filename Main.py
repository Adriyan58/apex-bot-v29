# ==============================================================================
# APEX ALPHA MASTER PRODUCTION BOT v34.2 (ENGLISH PRODUCTION EDITION)
# Broker: Goat Funded Trader MT5 | Multi-Market Strategy | Dynamic Lot Engine
# Features: Auto Balance Sync | Real-Time Market Price Test Trade | Session Engine
# ==============================================================================
...
# ==============================================================================
# 🏛️ GOAT FUNDED MT5 WEB CONNECTOR WITH REAL-TIME MARKET PRICE & BALANCE SYNC
# ==============================================================================
class GoatFundedWebAPI:
    ...
    async def get_live_market_price(self, session, symbol):
        """Fetches real-time live market price directly from broker quote feed."""
        ...
        try:
            async with session.get(price_url, headers=headers, timeout=5) as response:
                if response.status == 200:
                    data = await response.json()
                    return float(data.get("ask", 2420.50))
        except Exception as e:
...
def home():
    ...
    <div style="font-family: sans-serif; text-align: center; padding: 40px; background: #0d1117; color: #c9d1d9;">
        <h1 style="color: #58a6ff;">⚡ APEX ALPHA MASTER BOT v34.2</h1>
...
# ==============================================================================
# 🧪 INSTANT TEST TRADE ENGINE (EXECUTED AT LIVE REAL-TIME MARKET PRICE)
# ==============================================================================
async def trigger_instant_test_trade(session, goat_api):
    ...
    # Calculate Lot size dynamically using synced balance
    ...
    logging.info(f"🧪 Executing Test Trade on {test_symbol} at Live Price: {live_price}")
    ...
    # Telegram Notification (Not added to DAILY_TRADE_COUNT)
    msg = (
        ...
        f"ℹ️ *Note:* `Diagnostic test trade executed at real market price (Excluded from daily limit).`"
    )
...
async def main_trading_loop(session):
    ...
    startup_msg = (
        ...
        f"🧪 *Status:* `Market Scan Active. Initializing Live Test Trade in 60 seconds...`"
    )
    await send_telegram_async(session, startup_msg)

    # Launch Instant Test Trade
