# ==============================================================================
# APEX ALPHA MASTER PRODUCTION BOT v35.0 (STRICT DYNAMIC ENGINE)
# Broker: Goat Funded Trader MT5 | Multi-Market Strategy | Dynamic Lot Engine
# Features: Real Live Balance Sync | Live Market Price Test Trade | Strict Rules
# ==============================================================================
...
# ==============================================================================
# 🎯 DYNAMIC LOT SIZE CALCULATOR (NO FIXED BALANCE)
# ==============================================================================
...
# ==============================================================================
# 🏛️ GOAT FUNDED CONNECTOR (FETCHES REAL LIVE BALANCE)
# ==============================================================================
class GoatFundedWebAPI:
    ...
    async def get_live_market_price(self, session, symbol):
        """Fetches real-time price feed directly from broker."""
...
def home():
    ...
    <div style="font-family: sans-serif; text-align: center; padding: 40px; background: #0d1117; color: #c9d1d9;">
        <h1 style="color: #58a6ff;">⚡ APEX ALPHA MASTER BOT v35.0</h1>
...
async def trigger_single_test_trade(session, goat_api):
    ...
    # Calculate Lot Size dynamically based on live synced balance
    ...
    logging.info(f"🧪 Executing Single Diagnostic Test Trade | Synced Balance: ${goat_api.account_balance} | Lot: {dynamic_lot}")
    ...
    msg = (
        ...
        f"ℹ️ *Note:* `Single diagnostic test trade complete. Strict rule engine is active.`"
    )
...
async def main_trading_loop(session):
    ...
    startup_msg = (
        ...
        f"🧪 *Status:* `Engine ready. Initializing single test trade in 60s...`"
    )
    await send_telegram_async(session, startup_msg)

    # Trigger exactly ONE test trade
